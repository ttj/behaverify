...
move_toward_key::move_toward_key(const std::string& name, const BT::NodeConfiguration& config) : BT::SyncActionNode(name, config) {

}

BT::PortsList move_toward_key::providedPorts() {
...
BT::NodeStatus move_toward_key::tick() {
    ...
    agent_x = ((agent_x < 1) ? (agent_x + 1) : ((agent_x > 1) ? (agent_x - 1) : (agent_x)));
    agent_y = (((agent_x == 1) && (agent_y < 1)) ? (agent_y + 1) : (((agent_x == 1) && (agent_y > 1)) ? (agent_y - 1) : (agent_y)));
