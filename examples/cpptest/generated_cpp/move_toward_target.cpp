...
move_toward_target::move_toward_target(const std::string& name, const BT::NodeConfiguration& config) : BT::SyncActionNode(name, config) {

}

BT::PortsList move_toward_target::providedPorts() {
...
BT::NodeStatus move_toward_target::tick() {
    ...
    agent_x = ((agent_x < 4) ? (agent_x + 1) : ((agent_x > 4) ? (agent_x - 1) : (agent_x)));
    agent_y = (((agent_x == 4) && (agent_y < 4)) ? (agent_y + 1) : (((agent_x == 4) && (agent_y > 4)) ? (agent_y - 1) : (agent_y)));
